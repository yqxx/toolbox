...
function process() {
  ...
  const result =
    mode.value === 'format'
      ? formatJson(input.value, indent.value)
  ...
  <ToolPage>
    <div class="tool-panel">
      <div class="formatter-toolbar">
        <SegmentTabs v-model="mode" :tabs="modeTabs" aria-label="JSON 处理模式" />
      ...
      <div class="tool-grid-2" style="margin-top: 1.5rem">
        <div>
          ...
          <textarea
            ...
            placeholder='输入 JSON，如 {"name":"Toolbox","tools":["url","timestamp"]}'
            spellcheck="false"
          />
        </div>
        <div>
          <label class="field-label" for="json-output">输出</label>
          <textarea
            id="json-output"
            :value="output"
            class="field-textarea field-textarea--code"
